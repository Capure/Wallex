...
window.addEventListener("enableAudio", () => {
  navigator.mediaDevices
    .getUserMedia({
      ...
      video: {
        // This has to be here cuz reasons
        mandatory: {
          chromeMediaSource: "desktop",
          maxWidth: 1,
          maxHeight: 1,
    ...
    .then((stream) => {
      ...
      setInterval(() => {
        ...
        freqArray.forEach((raw) =>
          convertedArray.push(parseInt(raw.toString()) / 1024)
        ); // Left channel
        freqArray.forEach((raw) =>
          convertedArray.push(parseInt(raw.toString()) / 1024)
        ); // Right channel
...
HTMLCanvasElement.prototype.addEventListener = (eName, listener) =>
  window.addEventListener(eName, listener);
...
window.addEventListener("DOMContentLoaded", () => {
  const waitForWallpaper = setInterval(() => {
    ...
    ipcRenderer.on("update-props", () =>
      window.wallpaperPropertyListener.applyUserProperties(
        project.general.properties
      )
    ); // Allows wallex to update the props while the wallpaper is running
    window.wallpaperPropertyListener.applyUserProperties(
      project.general.properties
    );
