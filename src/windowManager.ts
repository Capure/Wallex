...
export class WindowManager {
  private windows: Map<
    number,
    { window: BrowserWindow | null; wallpaperPath: string }
  > = new Map();
  ...
  private loadRuntimeSettingsFromProject(project: any) {
    ...
    return project && project.wallex
      ? { ...runtimeSettings, ...project.wallex }
      : runtimeSettings;
  }
  public createNewWindow(screenId: number, wallpaper: Wallpaper) {
    ...
    if (
      ...
      const description = !project
        ...
        : 'The wallpaper must be of type "web"!';
    ...
    newWindow.loadFile(
      projectIsVideo
        ? path.join(__dirname, "..", "public", "video", "index.html")
        : path.join(wallpaper.path, project.file)
    );
    if (projectIsVideo) {
    ...
    if (project.wallex && project.wallex.injectLocalStorage) {
      // injecting key : value pairs into localStorage
      newWindow.webContents.once("did-navigate-in-page", () => {
        project.wallex.injectLocalStorage.forEach(
          (localStoragePair: { key: string; value: string }) => {
            newWindow.webContents.executeJavaScript(
              `localStorage.setItem("${localStoragePair.key}", '${localStoragePair.value}')`
            );
          }
        );
      });
    ...
    electronWallpaper.attachWindow(
      newWindow,
      this.screenManager.getOffsetX(screenId),
      jsOffsetY,
      width,
      height
    );
    if (!projectIsVideo && !runtimeSettings.disableMouseEvents) {
      mouseEventsNative.createMouseForwarder(newWindow, jsOffsetX, jsOffsetY);
    }
    const { width: safeWidth, height: safeHeight } = screen.workAreaSize;
    attachCpuSaver.attachCpuSaver(
      newWindow,
      projectIsVideo,
      jsOffsetX,
      jsOffsetY,
  ...
  public destroyWindowByScreenId(id: number) {
    ...
    if (!window) {
      throw Error("No BrowserWindow open for this screen!");
    }
    if (window.window) {
      window.window.close();
    }
